...
export function HeaderUserMenu({ user }: HeaderUserMenuProps) {
  ...
  return (
    <div className="flex items-center gap-4" ref={ref}>
      <ThemeToggle className="h-10 w-10 border-border/70 bg-black/5 backdrop-blur-sm" />
      
      <div className="relative">
        <button
          onClick={() => setOpen((v) => !v)}
          className={cn(
            "flex items-center gap-4 rounded-2xl border border-border/70 bg-[hsl(var(--panel-raised))] p-2 transition-all hover:border-primary/45",
            open && "border-primary/45 ring-2 ring-primary/10"
          )}
        >
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-primary to-[hsl(var(--amber-deep))] font-display text-base font-bold text-primary-foreground shadow-[0_4px_12px_-4px_hsl(var(--primary))]">
            {initials}
          </div>
